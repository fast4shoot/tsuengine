  ...
  CCenteredWindow(vec2d(500,350), "Stahování map")
{
  ...
  try{
    ...
    CText* tmp = new CText(vec2d(20, 50.), getSize()-vec2d(40., 70.), 14.);
...
void CDownloadMaps::doDownload(int){
  try{
    if(m_dataDownloadStatus == DDS_NONE){
      ...
      async_client::request request("http://tsuengine.php5.cz/data/dataLists/"+m_list->getSelectedString());
      request << net::header("Connection", "close");
...
void CDownloadMaps::think(){
  try{
    if(m_dataDownloadStatus != DDS_NONE){
      engine->log("m_dataDownloadStatus != DDS_NONE");
      if(ready(m_response)){
        engine->log("ready(m_response)");
        if(status(m_response) == 200){
          engine->log("status(m_response) == 200");
          String data = body(m_response);
          if(m_dataDownloadStatus == DDS_FILELIST){
            engine->log("m_dataDownloadStatus == DDS_FILELIST");
            json::mValue val;
            json::read(data, val);
            engine->log("data read");
            ...
            engine->log("files parsed and added");
            m_lastFile = -1;
            downloadNextFile();
...
void CDownloadMaps::downloadNextFile(){
  ...
  if(m_lastFile >= m_fileList.size()){
    ...
    async_client::request request("http://tsuengine.php5.cz/data/"+m_fileList[m_lastFile]);
    request << net::header("Connection", "close");
    m_response = m_client.get(request);
