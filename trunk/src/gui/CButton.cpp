...
CAbstractButton(pos,size)
{
  ...
  label =new CLabel(vec2d(4., getH()/15.), vec2d(getW()-10., getH()-getH()/3.),text, false, CLabel::ALIGN_CENTER);
  label2 = new CLabel(label->getPosition()+vec2d(1., 1.), label->getSize(), text, false, CLabel::ALIGN_CENTER);
