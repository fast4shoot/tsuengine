...
CWindow::CWindow(const vec2d& position, const vec2d& size, const String& label, double bottomHeight, bool hasTitle):
  ...
  m_title(hasTitle)
{
  ...
  if(m_title){
    ...
    addChild(_content=new CGuiPanel(vec2d(0., 30.), vec2d(getW(), getH()-30.)));
  }else{
    addChild(_content=new CGuiPanel(vec2d(0., 0.), vec2d(getW(), getH())));
  }
}
...
void CWindow::draw(){
  ...
  if(m_title){
    ...
    drawVerticalGradient(1., 1.,  getW()-2, getH()-2 -m_bottomHeight,rgba(.96,.96,.96,.8),bgColor);
  }
