...
void CGuiPanel::drawChildren(){
  int size=children.size();
  for (int i=0; i<size; i++){
    if(children.at(i)->getVisible()){
      glPushMatrix();
      glTranslatef(children.at(i)->getX(),children.at(i)->getY(),0.);
      children.at(i)->draw();
      children.at(i)->drawChildren();
      glPopMatrix();
    }
  }
}
...
bool CGuiPanel::handleMouseClick(const vec2d& position, const MouseButton button, const bool up){
  for(int i=children.size()-1;i>=0;i--){
    ...
    if( children.at(i)->isPointInside( positionInChild ) ){
...
void CGuiPanel::handleMouseMove(const vec2d& newPosition, const bool mouseOver){
  ...
  for(int i=children.size()-1;i>=0;i--){
    positionInChild = newPosition-(children.at(i)->getPosition());
    if(children.at(i)->isPointInside( positionInChild ) && !itemFound){
      children.at(i)->handleMouseMove( positionInChild, true);
      itemFound=true;
    }else{
      if(children.at(i)->getMouseOver()){
        children.at(i)->handleMouseMove( positionInChild, false);
      }
    }
  }
...
void CGuiPanel::mouseMove(const vec2d& newPosition,const bool mouseOver){
}
